...
export default function ProfilePage() {
    ...
    const fetchProfile = async () => {
        ...
        try {
            ...
            setCertificates(data.user.certificates ? data.user.certificates.split(',') : [])
        } catch (error) {
    ...
    const handleUpdateCoachProfile = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (newCertFiles) {
                Array.from(newCertFiles).forEach(file => {
                    formData.append('certificates', file)
                })
            }

            await userApi.updateCoachProfile(formData)
            toast.success('Cập nhật hồ sơ HLV thành công!')
            setNewCertFiles(null)
            fetchProfile()
        } catch (error) {
            toast.error('Cập nhật thất bại')
        } finally {
    ...
    const handleDeleteCert = async (certUrl: string) => {
        toast.info('Chức năng xóa chứng chỉ sẽ sớm được cập nhật')
    }
                        ...
                        <Card className="p-8 bg-card border-border shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                <input
                                    ...
                                    accept="image/*"
                                />
                                <h3 className="font-black text-xl mb-1">{user.name || 'User'}</h3>
                                ...
                                <div className="mt-4 w-full">
                                    <div className="p-3 bg-primary/5 border border-primary/10 rounded-2xl text-center">
                                        <p className="text-[10px] text-primary uppercase font-bold tracking-widest">Quyền hạn</p>
                                        <p className="font-black text-xs uppercase">{user.role}</p>
                                    </div>
                                </div>
                            </div>
                        </Card>
                        ...
                        {user.role === 'coach' && (
                            <Card className="p-8 bg-card border-border shadow-sm rounded-[2.5rem]">
                                <div className="flex items-center gap-2 mb-8">
                                    ...
                                    <h4 className="font-black text-xl text-accent">Hồ sơ chuyên môn</h4>
                                </div>

                                <form onSubmit={handleUpdateCoachProfile} className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-muted-foreground uppercase ml-2 tracking-wider">Giới thiệu bản thân (Bio)</label>
                                        <textarea
                                            rows={4}
                                            className="w-full bg-secondary/30 border-2 border-transparent focus:border-primary/20 focus:bg-card rounded-2xl px-5 py-3 text-sm font-medium transition-all outline-none resize-none"
                                            value={bio}
                                            onChange={(e) => setBio(e.target.value)}
                                            placeholder="Giới thiệu về kinh nghiệm và phong cách huấn luyện của bạn..."
                                        />
                                    </div>

                                    <div className="space-y-4">
                                        <label className="text-xs font-bold text-muted-foreground uppercase ml-2 tracking-wider">Bằng cấp & Chứng chỉ hiện có</label>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                            ...
                                            {certificates.length === 0 && (
                                                <p className="col-span-full text-xs text-muted-foreground italic">Chưa có chứng chỉ nào được tải lên.</p>
                                            )}
                                        </div>
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-muted-foreground uppercase ml-2 tracking-wider">Tải lên chứng chỉ mới</label>
                                        <div className="relative group">
                                            <input
                                                type="file"
                                                multiple
                                                accept="image/*,.pdf"
                                                onChange={(e) => setNewCertFiles(e.target.files)}
                                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                            />
                                            <div className={`p-6 border-2 border-dashed rounded-2xl transition-all text-center ${
                                                newCertFiles ? 'border-primary bg-primary/5' : 'border-border bg-secondary/10 group-hover:border-primary/50'
                                            }`}>
                                                <span className="text-2xl mb-2 block">📁</span>
                                                <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">
                                                    {newCertFiles ? `${newCertFiles.length} tệp mới đã chọn` : 'Thêm ảnh chứng chỉ hoặc PDF'}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-4 flex justify-end">
                                        <Button
                                            disabled={isSaving}
                                            type="submit"
                                            className="w-full sm:w-auto min-w-[200px] h-14 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl shadow-accent/20 bg-accent text-accent-foreground hover:scale-[1.02] active:scale-95 transition-all"
                                        >
                                            {isSaving ? 'Đang cập nhật...' : 'Cập nhật hồ sơ HLV'}
                                        </Button>
                                    </div>
                                </form>
                            </Card>
