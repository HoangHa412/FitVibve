...
export default function DashboardLayout({ children, navItems }: DashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-background font-sans selection:bg-primary/30 selection:text-primary">
      {/* Dynamic Background Blob Decorations with Floating Animation */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute -top-[10%] -left-[10%] w-[45%] h-[45%] bg-primary/8 rounded-full blur-[120px] animate-float-slow" />
        <div className="absolute top-[40%] -right-[10%] w-[35%] h-[50%] bg-accent/8 rounded-full blur-[110px] animate-float-reverse" />
      </div>
      ...
      <header className="glass sticky top-0 z-50 w-full animate-fade-in-down shadow-sm backdrop-blur-xl">
        <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            <Link
              href="/profile"
              className={`flex items-center gap-3 p-1.5 pr-3.5 rounded-full transition-all duration-200 hover:bg-secondary/70 hover:shadow-sm active:scale-95 ${
                pathname === '/profile' ? 'bg-secondary ring-1 ring-primary/20' : ''
              }`}
            >
              <div className="w-9 h-9 rounded-full border-2 border-primary/30 overflow-hidden bg-muted flex items-center justify-center shadow-sm transition-transform group-hover:scale-105">
                {user.avatar_url ? (
                  <img
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-[9px] text-muted-foreground uppercase tracking-widest font-black opacity-75 flex items-center gap-1.5 mt-0.5">
            ...
            <button
              ...
              className="p-2.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded-xl transition-all duration-200 active:scale-90"
              title="Đăng xuất"
            >
            ...
            {navItems.map((item, idx) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`group flex items-center gap-3 px-4 py-3 rounded-2xl text-sm font-medium transition-all duration-300 active:scale-95 ${
                    isActive
                      ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/25 -translate-r-1 font-semibold'
                      : 'text-muted-foreground hover:bg-secondary/80 hover:text-foreground hover:translate-x-1.5'
                  }`}
      ...
      <div className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 glass px-6 py-3 rounded-full flex items-center gap-6 shadow-2xl z-50 border border-white/20 backdrop-blur-xl animate-fade-in-up">
          ...
          return (
            <Link
              ...
              className={`p-2.5 rounded-xl transition-all duration-300 active:scale-90 ${
                isActive ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/40 -translate-y-1 scale-110' : 'text-muted-foreground hover:text-foreground'
              }`}
            >
